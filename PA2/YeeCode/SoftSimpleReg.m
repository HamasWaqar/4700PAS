...
% epi{1}(40 :140,40:140)= c_eps_0*11.3; % Inclusion
% epi{1}(80:100,60:110)= c_eps_0; % Inclusion (x, y)
% epi{1}(40:140,80:100)= c_eps_0; % Inclusion (x, y)
% epi{1}(85 :95,85:95)= c_eps_0*11.3; % Inclusion

%Grading
% epi{1} = ones(nx{1},ny{1})*c_eps_0; % Inclusion
% epi{1}(10 :30,10:140)= c_eps_0*11.3; % Inclusion
% epi{1}(40:60,10:140)= c_eps_0*11.3; % Inclusion (x, y)
% epi{1}(80:100,10:140)= c_eps_0*11.3; % Inclusion (x, y)
% epi{1}(120:140,10:145)= c_eps_0*11.3; % Inclusion
...
bc{1}.s(1).xpos = nx{1}/(8) + 1; % x position
bc{1}.s(1).type = 'ss'; 
bc{1}.s(1).fct = @PlaneWaveBC; % creates a plane wave


bc{1}.NumS = 2; %Number of sources
bc{1}.s(2).xpos = nx{1}/(8) + 1; % x position
bc{1}.s(2).type = 'ss'; 
bc{1}.s(2).fct = @PlaneWaveBC; % creates a plane wave
...
y0 = yMax/2;
sty = 1.5*lambda;
bc{1}.s(1).paras = {mag,phi,omega,betap,t0,st,s,y0*1.25,sty,'s'};
